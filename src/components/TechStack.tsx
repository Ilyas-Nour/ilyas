...
export const TechStack = () => {
  return (
    <section id="skills" className="py-24 md:py-48 px-6 md:px-12 lg:px-24 bg-black relative">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-px bg-white/5 border border-white/5 rounded-3xl overflow-hidden">
          {skillCategories.map((category, idx) => (
            <motion.div
              ...
              className="bg-black p-8 md:p-10 space-y-8 md:space-y-10 group hover:bg-white/[0.01] transition-colors duration-700"
            >
              <div className="space-y-2">
                <span className="font-mono text-[10px] text-accent/80 tracking-widest">{category.tag}</span>
                <h4 className="text-xl md:text-2xl font-bold text-white group-hover:text-accent transition-colors">{category.title}</h4>
              </div>
              ...
              <ul className="space-y-4 md:space-y-5">
                {category.skills.map((skill) => (
                  <li key={skill} className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-sans font-light text-white/80 group-hover:text-white transition-colors tracking-wide">{skill}</span>
                  </li>
                ))}
              </ul>
            </motion.div>
