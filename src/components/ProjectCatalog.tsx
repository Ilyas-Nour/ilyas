      ...
      <div className="max-w-7xl mx-auto relative z-10 w-full">
        ...
        >
          ...
          <div className="max-w-xs">
            <p className="text-muted font-mono text-[10px] tracking-widest leading-relaxed uppercase opacity-60">
        ...
        <div className="grid grid-cols-1 gap-px bg-white/5 border border-white/5 overflow-hidden rounded-3xl">
          {projects.map((project, index) => (
            <motion.a
              ...
              className="group relative bg-black p-10 md:p-20 flex flex-col lg:flex-row lg:items-center justify-between gap-12 transition-all duration-700 hover:bg-white/[0.02]"
            >
              <div className="flex items-start gap-12 lg:col-span-8">
                <span className="font-mono text-accent/40 text-xl group-hover:text-accent transition-colors">
                  {project.id}
                </span>
                
                <div className="space-y-6">
                  <h3 className="text-5xl md:text-7xl font-bold tracking-tighter group-hover:translate-x-4 transition-transform duration-700">
                    {project.title}
                  </h3>
                  <div className="flex flex-wrap gap-3">
                    {project.stack.map(s => (
                      <span key={s} className="text-[10px] font-mono tracking-widest text-muted group-hover:text-accent transition-colors border border-white/10 px-3 py-1 rounded-full bg-white/5 group-hover:border-accent/40">
                        {s}
                      </span>
                    ))}
                  </div>
              ...
              <div className="lg:max-w-xs text-left lg:text-right space-y-6">
                <p className="text-muted font-sans font-light leading-relaxed group-hover:text-white transition-colors">
                  {project.description}
                </p>
                <div className="flex lg:justify-end gap-6 pt-4 grayscale group-hover:grayscale-0 transition-all opacity-20 group-hover:opacity-100">
                  <div className="space-y-1">
                    <div className="text-[8px] font-mono text-muted uppercase tracking-[0.2em]">Build_State</div>
                    <div className="text-[10px] font-mono text-accent uppercase tracking-widest">{project.stats.build}</div>
                  </div>
                  <div className="space-y-1">
                    <div className="text-[8px] font-mono text-muted uppercase tracking-[0.2em]">Core_Latency</div>
